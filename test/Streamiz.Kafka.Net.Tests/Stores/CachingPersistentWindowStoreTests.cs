...
// Add MergedSortedCacheWrappedWindowStoreKeyValueIteratorTest tests
// Create abstract class
// doing test with real kafka cluster
public class CachingPersistentWindowStoreTests
{
    ...
    public void ShouldPutFetchWithRealTimestampFromCache()
    {
        ...
        using var enumeratorB = cachingStore.Fetch(BytesKey("b"), now.GetMilliseconds()+4, now.GetMilliseconds()+4);
        Assert.IsTrue(enumeratorA.MoveNext());
        Assert.IsTrue(enumeratorB.MoveNext());
        Assert.AreEqual(BytesValue("a"), enumeratorA.Current.Value.Value);
    ...
    public void ShouldFetchAllWithinTimestampRange()
    {
        ...
        for (int i = 0; i < array.Length; i++)
        {
            ...
            VerifyWindowedKeyValue(
                enumerator.Current,
                new Windowed<Bytes>(BytesKey(str), new TimeWindow(now.AddMilliseconds(i).GetMilliseconds(), now.AddMilliseconds(i).GetMilliseconds() + WINDOW_SIZE)),
                str);
        }
    ...
    public void ShouldGetAllFromCache()
    {
        ...
        foreach (var s in array)
        {
            ...
            VerifyWindowedKeyValue(
                enumerator.Current,
                new Windowed<Bytes>(BytesKey(s), new TimeWindow(DEFAULT_TIMESTAMP, DEFAULT_TIMESTAMP + WINDOW_SIZE)),
                s);
        }
