...
namespace Streamiz.Kafka.Net.Mock.Kafka
{
    internal class MockTopic
    {
        ...
        public int AddMessage(byte[] key, byte[] value, int partition, long timestamp = 0, Headers headers = null)
        {
            return partitions[partition].AddMessageInLog(key, value, timestamp, headers);
        }
